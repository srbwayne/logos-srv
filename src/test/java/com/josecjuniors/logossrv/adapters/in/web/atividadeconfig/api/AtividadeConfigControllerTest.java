...
class AtividadeConfigControllerTest {
 ...
 @Test void create_withValidData_shouldReturn201AndBootstrapForm() throws Exception {
  ...
  mockMvc.perform(get("/api/atividades-config/{id}/formulario",id).header("Authorization","Bearer "+token)).andExpect(status().isOk()).andExpect(header().string("X-Activity-Form-Version","1")).andExpect(jsonPath("$.campos").isArray());
 }
 ...
 @Test void getMissing_shouldReturn404() throws Exception {
  mockMvc.perform(get("/api/atividades-config/{id}", UUID.randomUUID()).header("Authorization", "Bearer " + token))
          .andExpect(status().isNotFound());
 }

 @Test void listSearch_shouldReturnOnlyMatchingCanonicalActivityWithoutLegacyFields() throws Exception {
  ...
  mockMvc.perform(get("/api/atividades-config").header("Authorization", "Bearer " + token).param("searchTerm", "Corrida"))
 ...
 @Test void updateMetadata_shouldReturn200AndChangeCanonicalFields() throws Exception {
  var activity = repository.save(new AtividadeConfig(new AtividadeConfigId(), "Inicial", "Antes"));
  var request = new UpdateAtividadeConfigRequest("Atualizada", "Depois", null, null, null, null);
  mockMvc.perform(put("/api/atividades-config/{id}", activity.getId().getValue()).header("Authorization", "Bearer " + token)
                  .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request)))
          .andExpect(status().isOk()).andExpect(jsonPath("$.nome").value("Atualizada"))
          .andExpect(jsonPath("$.descricao").value("Depois"));
 }

 @Test void duplicateNames_onCreateAndUpdate_shouldReturn409() throws Exception {
  ...
  mockMvc.perform(post("/api/atividades-config").header("Authorization", "Bearer " + token).contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(create))).andExpect(status().isConflict());
  var second = repository.save(new AtividadeConfig(new AtividadeConfigId(), "Segunda", "Outra"));
  var update = new UpdateAtividadeConfigRequest("Duplicada", "Atualização", null, null, null, null);
  mockMvc.perform(put("/api/atividades-config/{id}", second.getId().getValue()).header("Authorization", "Bearer " + token)
                  .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(update))).andExpect(status().isConflict());
 }

 @Test void eachLegacyCreateField_shouldReturn410WithoutActivityOrForm() throws Exception {
  ...
  for (int i = 0; i < fields.length; i++) {
   ...
   mockMvc.perform(post("/api/atividades-config").header("Authorization", "Bearer " + token).contentType(MediaType.APPLICATION_JSON).content(body)).andExpect(status().isGone());
   org.assertj.core.api.Assertions.assertThat(repository.existsByNome(name)).isFalse();
  }
 }

 @Test void eachLegacyUpdateField_shouldReturn410WithoutChangingMetadata() throws Exception {
  var activity = repository.save(new AtividadeConfig(new AtividadeConfigId(), "Stable", "Before"));
  String[] fields = {"xpBase", "estresseBase", "diasParaPenalidade", "xpPerdaPorCiclo"};
 ...
 @Test void canonicalCreatedUnusedActivity_shouldDeleteActivityAndOwnedForm() throws Exception {
  var request = new CreateAtividadeConfigRequest("ToDelete", "Unused", null, null, null, null);
  var result = mockMvc.perform(post("/api/atividades-config").header("Authorization", "Bearer " + token).contentType(MediaType.APPLICATION_JSON)
  ...
  mockMvc.perform(delete("/api/atividades-config/{id}", id).header("Authorization", "Bearer " + token)).andExpect(status().isNoContent());
  org.assertj.core.api.Assertions.assertThat(repository.findById(new AtividadeConfigId(id))).isEmpty();
  org.assertj.core.api.Assertions.assertThat(formularioRepository.findByAtividadeConfigId(new AtividadeConfigId(id))).isEmpty();
 }

 @Test void formReplacement_shouldIncrementVersionAndRejectStaleVersion() throws Exception {
  var request = new CreateAtividadeConfigRequest("Form", "Owned", null, null, null, null);
  var result = mockMvc.perform(post("/api/atividades-config").header("Authorization", "Bearer " + token).contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated()).andReturn();
  UUID id = UUID.fromString(result.getResponse().getHeader("Location").replaceAll(".*/", ""));
  String replacement = "{\"expectedVersion\":1,\"campos\":[]}";
  mockMvc.perform(put("/api/atividades-config/{id}/formulario", id).header("Authorization", "Bearer " + token)
                  .contentType(MediaType.APPLICATION_JSON).content(replacement)).andExpect(status().isOk())
          .andExpect(header().string("X-Activity-Form-Version", "2"));
  mockMvc.perform(put("/api/atividades-config/{id}/formulario", id).header("Authorization", "Bearer " + token)
                  .contentType(MediaType.APPLICATION_JSON).content(replacement)).andExpect(status().isConflict());
 }
}
